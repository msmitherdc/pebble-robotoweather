...
#define DATE_FRAME      (GRect(0, 58, 144, 168-62))
...
void success(int32_t cookie, int http_status, DictionaryIterator* received, void* context) {
	...
	memcpy(fcstcond_text, itoa(fcstcond_tuple->value->cstring), strlen(fcstcond_tuple->value->cstring));
...
//	layer_add_child(&window.layer, &text_sunset_layer.layer); 
	...
	// Add weather layer
	weather_layer_init(&weather_layer, GPoint(0, 80)); //0, 100
